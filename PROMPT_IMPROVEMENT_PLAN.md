...
> **탄소 배출 시나리오 생성기 프롬프트 최적화를 위한 종합 계획**
>
> 본 문서는 중고등학생 대상 교육용 AI 시나리오 생성 서비스의 프롬프트 품질 개선을 위한
> 체계적인 분석과 구체적인 개선 방안을 제시합니다.

## 📋 목차

1. [프로젝트 본질 분석](#프로젝트-본질-분석)
2. [프롤로그 생성 프롬프트 개선](#1-프롤로그-생성-프롬프트-개선)
3. [엔딩 생성 프롬프트 개선](#2-엔딩-생성-프롬프트-개선)
4. [검증 로직 설계](#3-검증-로직-설계-피드백-반영)
5. [이미지 프롬프트 개선](#4-이미지-프롬프트-생성-개선)
6. [우선순위별 구현 계획](#5-우선순위별-구현-계획-수정)
7. [예상 효과](#6-예상-효과)
8. [변경 이력](#변경-이력)
...
### 주요 개선 사항
1. **프롤로그**: 탄소배출 연관성 구체화, 자연스러운 표현 사용
2. **엔딩2(실패)**: 희망적 요소 제거, 후회와 절망 강조
3. **엔딩3(갈등)**: 정책 폐기 후 절망의 미래로 귀결
4. **검증 로직**: 유연하면서도 효과적인 입력 검증
5. **교육 효과**: 탄소배출 이해도 40% → 90% 목표
...
### Version 2.0 (최종 - 피드백 반영)
**날짜**: 2025-11-17

**주요 변경사항:**
1. **프롤로그 프롬프트**
   - ✅ 탄소배출 연관성 구체화를 최우선 개선 사항으로 설정
   - ✅ 자연스러운 표현 지침 추가 ("열다섯 살" → "중학생")
   - ✅ 인과관계 설명 강화 (탄소배출 → 환경변화 → 문제발생)

2. **엔딩 프롬프트**
   - ✅ 엔딩2(실패): "희망의 씨앗" 제거, "절망 속 후회" 강조
   - ✅ 엔딩3(갈등): 정책 폐기 후 절망의 미래로 귀결
   - ✅ 시간적 연속성 명시 (성공: 25-30년, 실패: 15-20년, 갈등: 5-10년)
   - ✅ 캐릭터 나이 변화 자동 처리 헬퍼 함수 추가

3. **검증 로직**
   - ✅ coreTheme 검증 추가 (기존에는 userSuggestion만 검증)
   - ✅ 유연한 검증 기준: 명백히 무관한 내용만 차단
   - ✅ 친절한 제안 메시지 제공
   - ✅ 과도한 제약 없이 학생의 자유로운 표현 지지
...
### 관련 파일
- `services/geminiService.ts`: 프롬프트 구현 위치
- `constants.ts`: ENDING_DETAILS promptInfo 정의
- `App.tsx`: 사용자 입력 검증 로직 위치
- `types.ts`: 데이터 타입 정의
...
프롬프트 개선에 대한 추가 의견이나 질문이 있으시면 언제든지 문의해주세요.
